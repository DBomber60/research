...
To make this concrete, let's use the following data set.


```{r, echo=FALSE}
data <- data.frame(
                drug = paste('drug',letters[1:8]),
...
Conceptually, this model assumes that each experiment provides an independent estimate of the drug's *true* effect, $\mu$.

Under this model, we can obtain an estimator $\hat{\mu}$ by maximizing the likelihood of our data.
...
While we are uncomfortable treating the effect parameter estimated by each experiment as being exactly equal ($\mu$), it also does not seem right to treat them as independent parameters, which would be the implied assumption under the following model,
